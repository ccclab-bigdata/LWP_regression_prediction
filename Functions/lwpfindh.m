...
for i = 1 : nTry
    [critEval(i), results(i,3), results(i,4)] = lwpeval(Xtr, Ytr, ...
        lwpparams(params.kernel, params.degree, params.useKNN, hTry(i), ...
        params.robust, params.knnSumWeights, params.standardize, params.safe),...
        crit, Xv, Yv, weights, true, false);
    if verbose
        if isRequiresL(crit)
...
for i = 1 : nTry
    [critEval, results3, results4] = lwpeval(Xtr, Ytr, ...
        lwpparams(params.kernel, params.degree, params.useKNN, ...
        hTry(i), params.robust, params.knnSumWeights, params.standardize, params.safe), ...
        crit, Xv, Yv, weights, true, false);
    if verbose
        if isRequiresL(crit)
            if (i > 1) && (hTry(i) ~= hTry(i-1)) && ...
