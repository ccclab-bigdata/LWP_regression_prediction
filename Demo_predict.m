...
params = lwpparams('GAU', 2, false); %% knn true is not for GAU GAR
...
Yq1 = reshape(Yq, size(gridX1));
figure;
surf(gridX1, gridX2, Yq1);
axis([-1 1 -1 1 80 200]);
hold on;
...
[hBest, critBest, results] = lwpfindh(X, Y, params, 'CV');
params = lwpparams('GAU', 2, false, hBest);
[gridX1, gridX2] = meshgrid(-0.5:0.2:0.5, -0.5:0.2:0.5);
Xp = [reshape(gridX1, numel(gridX1), 1) reshape(gridX2, numel(gridX2), 1)];
[Yp,Lp] = lwppredict(X, Y, params,Xp);
Yp1 = reshape(Yp, size(gridX1));
figure;
surf(gridX1, gridX2, Yp1);
hold on;
plot3(Xp(:,1), Xp(:,2), fun(Xp), 'r.', 'Markersize', 20);
figure;
surf(gridX1, gridX2, reshape(fun(Xp), size(gridX1)));
